...
const copyCode = () => { navigator.clipboard.writeText(code); setCopied(true);
...
const vote = (framework) => { if (userVoted) return; setVotes((prev) => ({
...prev, [framework]: prev[framework] + 1 })); setUserVoted(framework); };
...
Which framework do you prefer? </h4>
...
<div className='rounded-lg border border-gray-200 bg-white p-6 shadow-lg dark:border-gray-700 dark:bg-gray-800'>
<h4 className='mb-4 text-lg font-semibold text-gray-800 dark:text-white'> 📈
Simple Bar Chart </h4> <div className='flex h-32 items-end space-x-2'>
{data.map((item, index) => (
<div key={item.name} className='flex flex-1 flex-col items-center'> <div
className='w-full rounded-t bg-gradient-to-t from-blue-500 to-blue-300
transition-all duration-1000' style={{
                height: `${(item.value / maxValue) * 100}%`,
                animationDelay: `${index * 0.1}s`,
              }} ></div> <div className='mt-2 text-xs text-gray-600 dark:text-gray-400'>
{item.name} </div>
