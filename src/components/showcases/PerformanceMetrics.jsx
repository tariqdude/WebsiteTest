...
import { Activity, Zap, Globe, Clock, Monitor, Cpu, HardDrive, Wifi } from 'lucide-react';
...
const PerformanceMetrics = () => {
  ...
  const calculateMetrics = useCallback(() => {
    return safeExecute(() => {
      ...
      const memoryUsage = Math.round((memoryInfo.usedJSHeapSize || 0) / 1048576);
      ...
      const interactionLatency = interactionEntries.length > 0
        ? Math.round(interactionEntries[0].processingStart - interactionEntries[0].startTime)
        : 0;
  ...
  const metricsData = useMemo(() => [
    {
      ...
      status: metrics.loadTime < 1000 ? 'excellent' : metrics.loadTime < 3000 ? 'good' : 'poor'
    },
    {
      icon: Activity,
      label: 'DOM Nodes',
      ...
      status: metrics.domNodes < 1000 ? 'excellent' : metrics.domNodes < 5000 ? 'good' : 'heavy'
    },
    {
      icon: HardDrive,
      label: 'Memory',
      ...
      status: metrics.memoryUsage < 50 ? 'excellent' : metrics.memoryUsage < 100 ? 'good' : 'high'
    },
    {
      icon: Wifi,
      label: 'Connection',
      ...
      status: ['4g', '5g'].includes(metrics.connectionType) ? 'excellent' : 'standard'
    },
    {
      icon: Zap,
      label: 'Render Time',
      ...
      status: metrics.renderTime < 500 ? 'excellent' : metrics.renderTime < 1500 ? 'good' : 'slow'
    },
    {
      icon: Globe,
      label: 'Bundle Size',
      ...
      status: metrics.bundleSize < 500 ? 'excellent' : metrics.bundleSize < 1000 ? 'good' : 'large'
    },
    {
      icon: Monitor,
      label: 'Cache Hit',
      ...
      status: metrics.cacheEfficiency > 70 ? 'excellent' : metrics.cacheEfficiency > 40 ? 'good' : 'poor'
    },
    {
      icon: Cpu,
      label: 'Live FPS',
      ...
      status: liveMetrics.fps > 50 ? 'excellent' : liveMetrics.fps > 30 ? 'good' : 'low'
    }
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'poor': case 'slow': case 'high': case 'large': case 'low': 
