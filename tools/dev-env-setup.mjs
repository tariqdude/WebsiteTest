...
`;
    ...
    await execAsync(
      'npx husky add .husky/pre-commit "npm run lint && npm run check:types"'
    );
    ...
    console.log(
      chalk.yellow(
        '⚠️ Git hooks setup skipped (repository may not be initialized)'
      )
    );
  }
}
...
async function enhancePackageScripts() {
  ...
  try {
    ...
    const enhancedScripts = {
      ...
      'dev:analyze':
        'npm run build && npx astro build --experimental-static-build',
      'test:watch': 'vitest',
    ...
    console.log(
      chalk.green('✅ Package.json enhanced with development scripts')
    );
  } catch (error) {
...
async function main() {
  try {
    ...
    console.log(chalk.cyan('\n📚 Available commands:'));
    console.log(
      chalk.white('• npm run dev:debug    - Debug mode with verbose output')
    );
    console.log(
      chalk.white(
        '• npm run dev:profile  - Profile performance during development'
      )
    );
    ...
    console.log(
      chalk.white('• npm run supercharge  - Full optimization and build')
    );

    console.log(chalk.cyan('\n🚀 Next steps:'));
    console.log(
      chalk.white(
        '1. Install VS Code extensions: code --install-extension astro-build.astro-vscode'
      )
    );
