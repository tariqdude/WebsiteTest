...
async function main() {
    ...
    if (!selectedModule) {
        log(`Unknown command: "${command}". Use "help" to see available commands.`, ICONS.cross);
        process.exit(1);
    }
    ...
    log(`\n${ICONS.rocket} Task "${command}" finished in ${duration.toFixed(2)}s. Performed ${operations} operations.`, ICONS.sparkles);
}
